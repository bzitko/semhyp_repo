...
from graphbrain.hyperedge import Hyperedge as Edge
...
def build_unique_atom(text, *parts):
    atom = build_atom(text, *parts)
    return UniqueAtom(atom)
...
def _build_empty_atom(label, typesubtype, roles="", morph="", ents=""):
    ...
    while parts and parts[-1] == "":
        parts.pop()
    return build_unique_atom(label, ".".join(parts))

def _main_parse(sent, lemmas=False, debug=False):
    beta = {}
    types = {}
    atom2tok = {}
    tok2atom = {}
    ...
    for child in token_seq:
        atom = build_unique_atom(child.lemma_ if lemmas else child.text, build_part(child))
        atom2tok[atom] = child
        tok2atom[child] = atom
        beta[child] = atom
        hist[child] = [atom]
...
def _aux_parse(doc_or_sent, lemmas=False, debug=False):
    ...
    for sent in sentences:
        ...
        res = _main_parse(sent, lemmas=lemmas, debug=debug)
...
def parse(doc_or_sent, lemmas=False, debug=False):
    data = _aux_parse(doc_or_sent, lemmas=lemmas, debug=debug)
    if isinstance(doc_or_sent, Doc):
